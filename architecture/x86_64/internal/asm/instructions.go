...
// OperandType represents the type of operand an instruction expects
...
const (
	...
	OperandRegMem8              // 8-bit register or memory
	OperandRegMem16             // 16-bit register or memory
	OperandRegMem32             // 32-bit register or memory
	OperandRegMem64             // 64-bit register or memory
)

// InstructionEncoding represents how an instruction is encoded
type InstructionEncoding int

const (
	EncodingLegacy InstructionEncoding = iota // Legacy encoding
	EncodingVEX                               // VEX prefix encoding (AVX)
	EncodingEVEX                              // EVEX prefix encoding (AVX-512)
	EncodingXOP                               // XOP prefix encoding
)

// Prefix represents instruction prefixes
type Prefix byte

const (
	PrefixNone        Prefix = 0x00
	PrefixLock        Prefix = 0xF0 // LOCK prefix
	PrefixRepNE       Prefix = 0xF2 // REPNE/REPNZ prefix
	PrefixRep         Prefix = 0xF3 // REP/REPE/REPZ prefix
	...
	PrefixOperandSize Prefix = 0x66 // Operand-size override
	PrefixAddressSize Prefix = 0x67 // Address-size override
	PrefixREX         Prefix = 0x40 // REX prefix base (REX.W = 0x48)
)
...
// Instruction represents an x86_64 instruction
...
var (
	MOV = Instruction{
		...
		Forms: []InstructionForm{
			// MOV r8, r8
			{Operands: []OperandType{OperandReg8, OperandReg8}, Opcode: []byte{0x88}, ModRM: true, Encoding: EncodingLegacy},
			// MOV r16, r16
			{Operands: []OperandType{OperandReg16, OperandReg16}, Opcode: []byte{0x89}, ModRM: true, Encoding: EncodingLegacy},
			// MOV r32, r32
			{Operands: []OperandType{OperandReg32, OperandReg32}, Opcode: []byte{0x89}, ModRM: true, Encoding: EncodingLegacy},
			// MOV r64, r64
			{Operands: []OperandType{OperandReg64, OperandReg64}, Opcode: []byte{0x89}, ModRM: true, Encoding: EncodingLegacy, REXPrefix: 0x48},
			// MOV r8, imm8
			{Operands: []OperandType{OperandReg8, OperandImm8}, Opcode: []byte{0xB0}, Imm: true, Encoding: EncodingLegacy},
			// MOV r32, imm32
			{Operands: []OperandType{OperandReg32, OperandImm32}, Opcode: []byte{0xB8}, Imm: true, Encoding: EncodingLegacy},
			// MOV r64, imm64
			{Operands: []OperandType{OperandReg64, OperandImm64}, Opcode: []byte{0xB8}, Imm: true, Encoding: EncodingLegacy, REXPrefix: 0x48},
		},
	}
	...
	MOVZX = Instruction{
		...
		Forms: []InstructionForm{
			// MOVZX r32, r8
			{Operands: []OperandType{OperandReg32, OperandReg8}, Opcode: []byte{0x0F, 0xB6}, ModRM: true, Encoding: EncodingLegacy},
			// MOVZX r32, r16
			{Operands: []OperandType{OperandReg32, OperandReg16}, Opcode: []byte{0x0F, 0xB7}, ModRM: true, Encoding: EncodingLegacy},
		},
	}
