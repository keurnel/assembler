...
func (p *Parser) Parse() error {
	...
	// Global group initialization
	//
	globalGroup := InstructionGroup{
	...
	p.groups[GroupGlobalName] = globalGroup

	// Runtime group initialization (for built-in functions and system calls)
	//
	runtimeGroup := InstructionGroup{
	...
	p.groups[GroupRuntimeName] = runtimeGroup
	...
	for i < len(tokens) {
		...
		if token.Type == NAMESPACE {
			...
			ns := "namespace:" + token.Literal

			// A namespace can only be defined once, so we check if it already exists before
			// adding it to the groups map. If it already exists, we return an error to prevent
			// overwriting the existing namespace creating confusion and potential bugs in the code.
			//
			if _, exists := p.groups[ns]; exists {
				return errors.New("Parse error: Namespace '" + token.Literal + "' is already defined")
			}
		...
		if token.Type == USE {
			...
			// Use global namespace if no current namespace exists
			if currentNamespace == nil {
				globalGroup, exists := p.groups["global"]
				if !exists {
					globalGroup = InstructionGroup{
						Type:         INSTRUCTION_GROUP_TYPE_GLOBAL,
						Identifier:   "global",
				...
				globalGroup.Uses = append(globalGroup.Uses, "namespace:"+token.Literal)
				p.groups["global"] = globalGroup
			}
